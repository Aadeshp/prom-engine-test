...
type QueryStats struct {
	...
	Expression expressionStats `json:"stats,omitempty"`
}
...
type expressionStats struct {
	// The max number of input series to aggregation expressions
	// For example, if an aggregation rolls up N series into K series, this value would be N.
	AggOpInputSeriesMax int `json:"aggOpInputSeriesMax,omitempty"`
	// The max number of output series from aggregation expressions.
	// For example, if an aggregation rolls up N series into K series, this value would be K.
	AggOpOutputSeriesMax int `json:"aggOpOutputSeriesMax,omitempty"`
	// The max number of input series to vector-scalar binary expressions
	// For example, if an expression like `mymetric > 0` sees N series and reduces it to K,
	// this value would be N.
	VectorScalarBinOpInputSeriesMax int `json:"vsbOpInputSeriesMax,omitempty"`
	// The max number of output series from vector-scalar binary expressions
	// For example, if an expression like `mymetric > 0` sees N series and reduces it to K,
	// this value would be K.
	VectorScalarBinOpOutputSeriesMax int `json:"vsbOpOutputSeriesMax,omitempty"`
}

// NB: Still named QueryTimers despite expressionStats, to minimize the diff
// with upstream Prometheus.
...
func NewQueryTimers() *QueryTimers {
	return &QueryTimers{NewTimerGroup(), expressionStats{}}
}
