...
func (f *fanoutAppender) Append(typ textparse.MetricType, ref uint64, l labels.Labels, t int64, v float64) (uint64, error) {
	ref, err := f.primary.Append(typ, ref, l, t, v)
	if err != nil {
		return ref, err
	}

	for _, appender := range f.secondaries {
		if _, err := appender.Append(typ, ref, l, t, v); err != nil {
			return 0, err
		}
	}
	return ref, nil
