...
// parseGraphiteDefault sets the default graphite parse level. Used for testing.
var parseGraphiteDefault bool

// MustSetParseGraphiteDefault is a test function to force graphite option.
func MustSetParseGraphiteDefault(v bool) { parseGraphiteDefault = v }
...
// ParseExprWithGraphiteSelectors returns the expression parsed from the input,
// attempting to parse "graphite-style" metric selectors (i.e. those containing
// one or more of the following characters: [-*.] )
func ParseExprWithGraphiteSelectors(input string) (expr Expr, err error) {
	return parseExpr(input, true)
}

// ParseGraphiteExprNoValidation returns the expression from the input without any validation.
func ParseGraphiteExprNoValidation(input string) Expr {
	result := GraphiteExpr{
		GraphiteExpr: input,
		EndPos:       PositionRange{Start: 0, End: Pos(len(input))},
	}
	return &result
}

func parseExpr(input string, withGraphite bool) (expr Expr, err error) {
	p := newParser(input)
	if withGraphite {
		p.lex.parseGraphite = true
	}
...
func ParseMetricSelector(input string) (m []*labels.Matcher, err error) {
	return parseMetricSelector(input, false)
}

// ParseMetricSelectorWithGraphiteSelectors parses the provided textual metric
// selector into a list of label matchers, attempting to parse "graphite-style"
// metric selectors (i.e. those containing one or more of the following
// characters: [-*.] )
func ParseMetricSelectorWithGraphiteSelectors(input string) (m []*labels.Matcher, err error) {
	return parseMetricSelector(input, true)
}

func parseMetricSelector(input string, g bool) (m []*labels.Matcher, err error) {
	p := newParser(input)
	if g {
		p.lex.parseGraphite = true
	}
...
func newParser(input string) *parser {
	...
	p.lex = Lexer{
		input:         input,
		state:         lexStatements,
		parseGraphite: parseGraphiteDefault,
	}
	return p
}
