...
type Lexer struct {
	...
	parseGraphite   bool // determines if this should attempt to parse graphite.
	inNameCandidate bool // determines if the current string is a potential name candidate.
...
// nolint:unused
func (l *Lexer) isDashNameSelector() bool {
	if !l.parseGraphite || !l.inNameCandidate {
		return false
	}
	...
	defer func() {
		// Ensure lexer backs up.
		l.pos -= width
	}()

	for r := l.next(); r != eof; r = l.next() {
		width += l.width
		if l.isAlphaCandidate(r) || l.isGraphiteRune(r) {
			if infCandidate {
				if unicode.ToLower(r) == inf[infIdx] {
					infIdx++
					// This is the string `-Inf`, which should not be a name selector.
					if infIdx == len(inf) {
						return false
					}
				} else {
	...
	// Ensure at least one extra digit exists after this.
	return width > 0 && !digitOnly
}

func (l *Lexer) isStarNameSelector() bool {
	if !l.parseGraphite || !l.inNameCandidate {
		return false
	}
...
func (l *Lexer) isGraphiteRune(r rune) bool {
	return r == '.' || r == '*' || r == '-'
}

// isAlphaCandidate reports whether r is alphabetic, underscore, colon,
// dot, star, or dash.
func (l *Lexer) isAlphaCandidate(r rune) bool {
	if isAlpha(r) || r == ':' {
		return true
	}
...
// isNameCandidate reports whether r is alphabetic, digit, underscore, colon,
// dot, star, or dash.
func (l *Lexer) isNameCandidate(r rune) bool {
	if isAlphaNumeric(r) || r == ':' {
		return true
	}

	return l.parseGraphite && l.isGraphiteRune(r)
}
...
Loop:
	for {
		switch r := l.next(); {
		case l.isNameCandidate(r):
			// absorb
			l.inNameCandidate = true
		default:
			l.backup()
