...
func (ev *evaluator) eval(expr parser.Expr) (parser.Value, storage.Warnings) {
	...
	case *parser.Call:
		...
		if !ok {
			ev.error(fmt.Errorf("function %s is not implemented", e.Func.Name))
			return nil, nil
		}
...
func (ev *evaluator) VectorscalarBinop(op parser.ItemType, lhs Vector, rhs Scalar, swap, returnBool bool, enh *EvalNodeHelper) Vector {
	if ev.stats != nil && len(lhs) > ev.stats.Expression.VectorScalarBinOpInputSeriesMax {
		ev.stats.Expression.VectorScalarBinOpInputSeriesMax = len(lhs)
	}
	...
	if ev.stats != nil && len(enh.Out) > ev.stats.Expression.VectorScalarBinOpOutputSeriesMax {
		ev.stats.Expression.VectorScalarBinOpOutputSeriesMax = len(enh.Out)
	}
	return enh.Out
}
...
func (ev *evaluator) aggregation(op parser.ItemType, grouping []string, without bool, param interface{}, vec Vector, seriesHelper []EvalSeriesHelper, enh *EvalNodeHelper) Vector {
	if ev.stats != nil && len(vec) > ev.stats.Expression.AggOpInputSeriesMax {
		ev.stats.Expression.AggOpInputSeriesMax = len(vec)
	}
	...
	if ev.stats != nil && len(enh.Out) > ev.stats.Expression.AggOpOutputSeriesMax {
		ev.stats.Expression.AggOpOutputSeriesMax = len(enh.Out)
	}
	return enh.Out
}
